...
//   // ---------------------------------------------------------------------------
//   // Detail state — selected order + its linked bills and receiving notes
//   // ---------------------------------------------------------------------------

//   /// The currently selected purchase order (for master-detail UI).
//   final _selectedOrderSubject = BehaviorSubject<PurchaseOrder?>.seeded(null);

//   /// Bills linked to the selected purchase order.
//   final _orderBillsSubject =
//       BehaviorSubject<UnmodifiableListView<Bill>>.seeded(
//         UnmodifiableListView([]),
//       );

//   /// Receiving notes linked to the selected purchase order.
//   final _orderReceivingNotesSubject =
//       BehaviorSubject<UnmodifiableListView<ReceivingNote>>.seeded(
//         UnmodifiableListView([]),
//       );

//   /// Whether detail data is loading.
//   final _detailLoadingSubject = BehaviorSubject<bool>.seeded(false);
...
//   /// Gets the selected order subject.
//   BehaviorSubject<PurchaseOrder?> get selectedOrderSubject =>
//       _selectedOrderSubject;

//   /// Gets the order bills subject.
//   BehaviorSubject<UnmodifiableListView<Bill>> get orderBillsSubject =>
//       _orderBillsSubject;

//   /// Gets the order receiving notes subject.
//   BehaviorSubject<UnmodifiableListView<ReceivingNote>>
//   get orderReceivingNotesSubject => _orderReceivingNotesSubject;

//   /// Gets the detail loading stream.
//   Stream<bool> get detailLoadingStream => _detailLoadingSubject.stream;
...
//   /// Selects a purchase order and loads its linked bills and receiving notes.
//   Future<void> loadOrderDetail(String purchaseOrderId) async {
//     try {
//       _detailLoadingSubject.add(true);

//       // Find in local list first for instant UI update
//       final localMatch = _purchaseOrdersSubject.value.firstWhere(
//         (o) => o.refId == purchaseOrderId,
//         orElse: () => PurchaseOrder(),
//       );
//       if (localMatch.hasRefId()) {
//         _selectedOrderSubject.add(localMatch);
//       }

//       final store = userPreferences.store;
//       if (store == null) return;

//       // Load bills and receiving notes in parallel
//       final results = await Future.wait([
//         BillRepository.instance.listBills(
//           storeId: store.refId,
//           purchaseOrderId: purchaseOrderId,
//         ),
//         ReceivingNoteRepository.instance.listReceivingNotes(
//           storeId: store.refId,
//           purchaseOrderId: purchaseOrderId,
//         ),
//       ]);

//       _orderBillsSubject.add(
//         UnmodifiableListView(results[0] as List<Bill>),
//       );
//       _orderReceivingNotesSubject.add(
//         UnmodifiableListView(results[1] as List<ReceivingNote>),
//       );
//     } on Exception catch (e) {
//       _logger.severe('loadOrderDetail Error: $e');
//     } finally {
//       _detailLoadingSubject.add(false);
//     }
//   }

//   /// Creates a bill from the currently selected purchase order.
//   Future<bool> createBillFromSelectedOrder({
...
//       final bill = await BillRepository.instance.createBillFromPurchaseOrder(
...
//       if (bill != null) {
//         final current = _orderBillsSubject.value.toList();
//         _orderBillsSubject.add(UnmodifiableListView([...current, bill]));
...
//       return false;
//     } on Exception catch (e) {
//       _logger.severe('createBillFromSelectedOrder Error: $e');
...
//   /// Deletes a bill by [billId] and refreshes the detail bills list.
//   Future<bool> deleteBill(String billId) async {
//     try {
//       final success = await BillRepository.instance.deleteBill(billId);
//       if (success) {
//         final current = _orderBillsSubject.value
//             .where((b) => b.refId != billId)
//             .toList();
//         _orderBillsSubject.add(UnmodifiableListView(current));
//       }
...
//   /// Clears the selected order detail (e.g. when navigating away on desktop).
//   void clearOrderDetail() {
//     _selectedOrderSubject.add(null);
//     _orderBillsSubject.add(UnmodifiableListView([]));
//     _orderReceivingNotesSubject.add(UnmodifiableListView([]));
//   }
...
//     _selectedOrderSubject.close();
//     _orderBillsSubject.close();
//     _orderReceivingNotesSubject.close();
//     _detailLoadingSubject.close();
//   }
// }
